...
import { ensureProxyToken } from './state.mjs';
...
const PROXY_TOKEN = ensureProxyToken();
let ws = null;
let cmdId = 0;
...
async function connect() {
  ...
  if (!chromePort) {
    ...
    if (!discovered) {
      throw new Error(
        '浏览器未开启远程调试端口。请用以下方式启动 Chromium 系浏览器：\n' +
...
function isAuthorized(req) {
  return req.headers['x-web-access-token'] === PROXY_TOKEN;
}

function rejectUnauthorized(res) {
  res.statusCode = 401;
  res.end(JSON.stringify({
    error: 'missing or invalid X-Web-Access-Token header',
  }));
}
