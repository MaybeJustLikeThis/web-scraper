...
import { ensureConfigFromTemplate, getConfigPath, getStateDir, proxyAuthHeaders } from './state.mjs';
...
function ensureConfigExists() {
  ...
  ensureConfigFromTemplate(CONFIG_TEMPLATE);
  if (!existed) console.log(`config: 已从模板创建 ${CONFIG_PATH}`);
}
...
function checkNode() {
  ...
  if (major >= 22) console.log(`node: ok (${version})`);
    ...
    console.log(`node: error (${version}, 需要 Node.js 22+)`);
    process.exit(1);
  }
}
...
function httpGetJson(url, timeoutMs = 3000) {
  return fetch(url, { headers: proxyAuthHeaders(), signal: AbortSignal.timeout(timeoutMs) })
