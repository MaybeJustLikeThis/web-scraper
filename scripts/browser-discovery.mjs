...
//   2. state/config.env 里 WEB_ACCESS_BROWSER 设了 → 严格模式，找不到则硬错
//   3. 都没设 → "ask" 模式，提示调用方询问用户
//
// 不擅自降级：偏好不可用一律硬错，让用户介入。
// 持久态只有 state/config.env 一处；override 是单次 spawn 通过命令行参数表达，不读 process.env。
...
import { getConfigPath } from './state.mjs';
...
// 读 state/config.env 文件（不写入 process.env，分清来源）
// 格式：KEY=VALUE，# 开头是注释
function readConfig() {
  const cfg = {};
  let content;
...
export async function selectBrowser(override = null) {
  ...
  // 2. state/config.env preference（持久）
  if (configured) {
